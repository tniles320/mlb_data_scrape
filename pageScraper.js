...
const scraperObject = {
	...
	async scraper(browser) {
		...
		let data = await page.$$eval(
			...
			(tableData) => {
				...
				tableData.map((data) => {
					teamData.push({
						teamId: parseInt(
							data.querySelector(".p-logo").getAttribute("src").match(/\d+/gi)[0]
						),
						team: data.querySelector(".team--name > a").getAttribute("data-team-name"),
						winStreak: '"' + data.querySelector(".col-7 > span").textContent + '"',
		...
		// functions.dbEntry(data); // Uncomment to make database entry
	},
};
